...
export function CourseForm({ initialValue = EMPTY_COURSE, submitLabel = "Add course", busy = false, onSubmit, onCancel }: CourseFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <label htmlFor={`${prefix}name`} className="text-sm font-semibold text-slate-700">Course name</label>
        <input id={`${prefix}name`} className={inputClass} required value={course.name} onChange={(event) => setCourse((current) => ({ ...current, name: event.target.value }))} placeholder="e.g. Advanced TypeScript" />
      </div>
      <div>
        <label htmlFor={`${prefix}description`} className="text-sm font-semibold text-slate-700">Description</label>
        <textarea id={`${prefix}description`} className={`${inputClass} min-h-24 resize-y`} required value={course.description} onChange={(event) => setCourse((current) => ({ ...current, description: event.target.value }))} placeholder="What do you want to learn?" />
      </div>
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <label htmlFor={`${prefix}target-date`} className="text-sm font-semibold text-slate-700">Target date</label>
        ...
        <div>
          <label htmlFor={`${prefix}status`} className="text-sm font-semibold text-slate-700">Status</label>
          <select id={`${prefix}status`} className={inputClass} value={course.status} onChange={(event) => setCourse((current) => ({ ...current, status: event.target.value as CourseInput["status"] }))}>
            {COURSE_STATUSES.map((status) => <option key={status}>{status}</option>)}
          </select>
        </div>
      </div>
      <div className="flex justify-end gap-3">
        {onCancel ? <button type="button" onClick={onCancel} className="rounded-xl px-5 py-3 text-sm font-semibold text-slate-600 hover:bg-slate-100">Cancel</button> : null}
